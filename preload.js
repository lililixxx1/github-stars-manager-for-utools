...
// ==================== 暴露给前端 ====================
// 进行中的 utools.ai 调用（批量分析并发最高 5 个），用于"停止分析"时立即中止 in-flight 调用（R1/H1）
...
window.githubStarsAPI = {
    ...
    // C2：同步维护笔记索引——getAllNotes 是索引驱动的，老用户（索引已存在）环境里
    // 仅写键会导致导入的笔记不可见；索引不存在（未迁移）时不新建，与 setNote 同一哲学，
    // 留给 getAllNotes 首次全量扫描懒建完整索引
    setNotes: (notes) => {
        const index = readNoteIndex();
        notes.forEach(n => {
            utools.dbStorage.setItem(getNoteKey(n.repoId), n);
            if (index && !index.includes(n.repoId)) {
                index.push(n.repoId);
            }
        });
        if (index) {
...
Return in JSON format: {"summary": "...", "tags": [...], "platforms": [...]}`;
        ...
        try {
            ...
            // 保留 PromiseLike 句柄，支持"停止分析"时立即中止进行中的调用（R1/H1）
            ...
            if (jsonMatch) {
                ...
                // 校验有效内容（R2）：summary 缺失或为空视为失败，进入冷却+熔断，
            ...
            // 无法解析出 JSON 视为失败（R2），向上抛出，由调用方标记 analysisFailed 进入冷却
            throw new Error('AI 返回内容无法解析为 JSON');
        } catch (error) {
            console.error('[AI分析] 调用失败:', error);
            // 不吞错误（R2）：额度耗尽/网络异常等必须让上层感知，否则会继续无意义地消耗额度
            throw error;
        }
    },

    // 中止所有进行中的 AI 分析调用（R1/H1，utools.ai 返回的 PromiseLike.abort()）
    abortAiCall: () => {
    ...
    // ========== AI 翻译缓存落盘（R6，同步 KV，杜绝 load-write 竞态） ==========
