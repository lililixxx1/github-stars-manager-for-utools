...
- 每页数量支持固定分页或"全部"展示（全部模式走虚拟滚动，大量仓库下仍保持流畅）
- 排序偏好记忆、复制仓库地址、筛选面板与列表键盘导航
- 中英文界面切换
...
- 启动加载与搜索筛选提速（搜索索引缓存 + 输入防抖 + 排序预计算）
- 仓库数据增量写入（只重写变更分片，替代整库落盘）
- 全量同步并发分页抓取与去重
- 首页虚拟滚动"全部"模式
- 暗色模式显示修复、设置页改版、操作提示支持撤销
...
- 大数据量仓库列表采用分片存储策略，降低单文档体积风险，并支持按分片增量写入
